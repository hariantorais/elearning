...
class Materi extends MY_Controller
{
    ...
    function index($segment_3 = '')
    {
        ...
        if (is_siswa()) {
            ...
            # ambil parentnya
            $filter['kelas_id'] = array($retrieve_kelas['id']);
        }
    ...
    function absen($segment_3 = '', $segment_4 = '', $segment_5 = '')
    {
        $materi_id = (int)$segment_3;

        // absen
        $siswa_id = get_sess_data('login', 'siswa_id');

        $materi = $this->materi_model->retrieve($materi_id);
        $absen = $this->absen_model->retrieve($materi_id);
        ...
        $this->twig->display('absen_siswa.html', $data);
    ...
    function hapus_absen()
    {
        ...
        $where = array ('id' => $id);
        $this->absen_model->hapus($where, 'absen_siswa');
        $this->session->set_flashdata('success', 'Hapus data berhasil.');
        redirect('materi/absen/'.$materi_id);
    }
    ...
    function excel(){
        ...
        $data['absen'] = $absen;
        $data['materi']['mapel'] = $this->mapel_model->retrieve($materi['mapel_id']);
        $data['materi']['pembuat'] = array('nama'      => $pengajar['nama']);

        header("Content-type: application/vnd-ms-excel");
        header("Content-Disposition: attachment; filename=RekapKehadiranSiswa-" . url_title($data['materi']['judul'], '_', true)  . ".xls");
        $this->twig->display('export-absen.html', $data);
    }
     ...
     function print(){
        ...
        $this->twig->display('print-materi.html', $data);
    }
