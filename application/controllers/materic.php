...
class Materi extends MY_Controller
{
    ...
    function detail($segment_3 = '', $segment_4 = '', $segment_5 = '')
    {
        ...
        // absen
        $siswa_id = get_sess_data('login', 'id');


        $sql = $this->db->query("SELECT siswa_id, materi_id FROM absen_siswa where siswa_id = '$siswa_id' and materi_id = '$materi_id'");
        $cek = $sql->num_rows();
        if ($cek > 0) {
            ...
            $result = $this->db->get('absen_siswa');
                ...
                $absen_id = $this->absen_model->create(
                    $siswa_id,
                    $materi_id,
                    $absen = 1
                );
